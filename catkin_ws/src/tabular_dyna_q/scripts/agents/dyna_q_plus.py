...
'''
@Author       : LI Jinjie
@Date         : 2020-05-10 08:40:08
@LastEditors  : LI Jinjie
@LastEditTime : 2020-05-10 08:53:42
@Units        : None
@Description  : the agent class of Dyna Q+
@Dependencies : None
@NOTICE       : None
'''
import numpy as np
...
class DynaQPlusAgent(BaseAgent):
    ...
    def planning_step(self):
        ...
        for i in range(self.planning_steps):
            ...
            s_, r = self.model[s][a]  # s,a,s',r
            r = r + self.kappa * np.sqrt(self.tau[s, a])
            if s_ == -1:  # terminal transition
                max_q = 0
            else:  # non-terminal transition
                max_q = max(self.q_values[s_])
            self.q_values[s, a] += self.step_size * \
                (r + self.gamma * max_q - self.q_values[s, a])
        ### END CODE HERE ###
    ...
    def agent_step(self, reward, state):
        ...
        self.update_model(self.past_state, self.past_action,
                          state, reward)  # s,a,s',r
        self.planning_step()
        self.past_state = state
        self.past_action = self.choose_action_egreedy(state)
        ### END CODE HERE ###
